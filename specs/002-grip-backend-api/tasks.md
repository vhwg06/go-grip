...
- [x] T004 Add GORM PostgreSQL dependencies and remove unused RPC transport dependencies in go.mod
...
- [x] T096 [US5] Implement notification usecase for unified inbox, unread count, mark read, mark all read, clear inbox, and translation-key payloads in internal/usecase/notification/center.go
...
- [x] T104 [P] Update quickstart and REST-only runtime notes in README.md
- [x] T105 [P] Update generated Swagger artifacts for all Grip Store REST routes in docs/swagger.yaml
- [x] T106 Remove obsolete RPC controller packages from internal/controller/amqp_rpc, internal/controller/grpc, and internal/controller/nats_rpc
- [x] T107 Remove obsolete RPC server packages from pkg/grpcserver, pkg/nats, and pkg/rabbitmq
- [x] T108 Remove obsolete RPC tests, generated protobuf tooling, and unused RPC dependencies from go.mod
- [x] T109 Run mock generation and commit generated mock updates in internal/usecase/mocks_repo_test.go
- [x] T110 Run formatting and import ordering across Go files with gofmt/goimports-equivalent tooling in internal
- [x] T111 Run full unit test suite and fix failures reported by go test ./... in go.mod
- [x] T112 Run integration test suite and fix failures reported by docker-compose-integration-test.yml
- [x] T113 Validate quickstart smoke tests and document any required environment changes in specs/002-grip-backend-api/quickstart.md
- [x] T114 [P] Add final security checklist notes for card-key masking, admin-only routes, blocked-user guards, and payment signature verification in specs/002-grip-backend-api/checklists/security.md
