...
import (
	...
	"github.com/evrone/go-clean-template/internal/module/media"
...
func TestMediaUseCaseValidation(t *testing.T) {
	t.Parallel()
	storage := &stubStorage{}
	uc := media.NewMediaUseCase(persistent.NewMediaRepo(nil), storage, media.Config{MaxBytes: media.MaxMediaUploadBytes})
	asset, err := uc.Store(context.Background(), media.MediaAsset{FileName: "a.jpg", MimeType: "image/jpeg", SizeBytes: 10})
	require.NoError(t, err)
	require.NotEmpty(t, asset.ID)
	_, err = uc.Store(context.Background(), media.MediaAsset{FileName: "a.gif", MimeType: "image/gif", SizeBytes: 10})
	require.ErrorIs(t, err, media.ErrInvalidInput)
}
...
func TestDelete(t *testing.T) {
	...
	t.Run("invalid uuid returns ErrInvalidInput", func(t *testing.T) {
		uc := media.NewMediaUseCase(persistent.NewMediaRepo(nil), &stubStorage{}, media.Config{MaxBytes: media.MaxMediaUploadBytes})
	...
	t.Run("existing media calls storage delete and repo delete", func(t *testing.T) {
		...
		err := repo.Store(context.Background(), &media.MediaAsset{
			ID:       mediaID,
			FileName: "test.jpg",
		...
		uc := media.NewMediaUseCase(repo, storage, media.Config{MaxBytes: media.MaxMediaUploadBytes})
