...
func mapOrdersError(err error) (int, openapi.ErrorResponse) {
	...
	if errors.Is(err, entity.ErrOrderNotFound) || errors.Is(err, entity.ErrNotFound) {
	...
	if errors.Is(err, entity.ErrRefundNotAllowed) || errors.Is(err, entity.ErrOrderStateConflict) {
		resp := openapi.ErrorResponse{}
		resp.Error.FromErrorPayload(openapi.ErrorPayload{
