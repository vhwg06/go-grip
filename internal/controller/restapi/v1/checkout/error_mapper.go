...
func mapCheckoutError(err error) (int, openapi.ErrorResponse) {
	...
	if errors.Is(err, entity.ErrNotFound) || errors.Is(err, entity.ErrOrderNotFound) {
	...
	if errors.Is(err, entity.ErrPaymentFailed) || errors.Is(err, entity.ErrPaymentInvalidSign) {
		resp := openapi.ErrorResponse{}
		resp.Error.FromErrorPayload(openapi.ErrorPayload{
			Code:    "PAYMENT_FAILED",
